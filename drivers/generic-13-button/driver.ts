...
module.exports = class GenericThirteenButtonDriver extends Homey.Driver {
  ...
  async onInit() {
    ...
    this.homey.flow.getActionCard('generic-13-button_on')
    ...
    this.homey.flow.getActionCard('generic-13-button_off')
      .registerRunListener(async (args) => {
        const { device } = args;
    ...
    this.homey.flow.getActionCard('generic-13-button_timer')
      .registerRunListener(async (args) => {
        const { device } = args;
        this.log(`[${this.constructor.name}] Timer flow action triggered`);
        await device.triggerCapabilityListener('button.timer', true);
      });

    this.homey.flow.getActionCard('generic-13-button_mode')
      .registerRunListener(async (args) => {
    ...
    this.homey.flow.getActionCard('generic-13-button_brightness')
      .registerRunListener(async (args) => {
